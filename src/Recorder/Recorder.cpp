...
void Recorder::start(const std::string& path)
{
    recording = true;
    frameHasData = false;
    currentFrame.resize(settings.recWidth * settings.recHeight * 3, 0);
    finishedLevel = false;
    renderer.width = settings.recWidth;
    renderer.height = settings.recHeight;
    renderer.begin();
    ...
    std::thread([&, path, songFile, fadeIn, fadeOut, bgVolume, sfxVolume, isTestmode, songOffset]() {
        std::stringstream ss;
        ss << '"' << ffmpegPath << '"' << " -y -f rawvideo -pix_fmt rgb24 -s " << settings.recWidth << "x" << settings.recHeight << " -r " << settings.recFps << " -i - ";
        if (!settings.recCodec.empty())
            ss << "-c:v " << settings.recCodec << " ";
        if (!settings.recBitrate.empty())
            ss << "-b:v " << settings.recBitrate << " ";
        if (!settings.recExtraArgs.empty())
            ss << settings.recExtraArgs << " ";
        else
            ss << "-pix_fmt yuv420p ";
        ss << "-vf \"vflip\" -an \"" << path << "\" ";
        ...
        auto total_time = lastFrameTime;
        {
            ...
            if (!settings.recExtraAudioArgs.empty())
                stream << settings.recExtraAudioArgs << " ";
...
void Recorder::updateSongOffset(gd::PlayLayer* playLayer) {
    songStartOffset = playLayer->m_levelSettings->m_songStartOffset + playLayer->timeForXPos2(
        playLayer->m_player1->m_position.x, playLayer->m_isTestMode);
}
...
void Recorder::handleRecording(gd::PlayLayer* play_layer, float dt) {
    if (!play_layer->m_hasLevelCompleteMenu || afterEndExtraTime < afterEndDuration) {
        ...
        auto frame_dt = 1. / static_cast<double>(settings.recFps);
